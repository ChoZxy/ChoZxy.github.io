...
class BoardWidget extends Pad {
    ...
    static final int
            ...
            OUT = 40,
            EH = 30,
            UH = 6 * SQUARE_SEP + SEPARATOR_SIZE;
    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        for (int n = 1; n <= _board.size(); n++) {
            for (int m = 1; m <= _board.size(); m++) {
                if (_board.get(n, m).getSide() == RED) {
                    g.setColor(RED_TINT);
                } else if (_board.get(n, m).getSide() == BLUE) {
                    g.setColor(BLUE_TINT);
                } else {
                    g.setColor(NEUTRAL);
                }
                g.fillRect(SEPARATOR_SIZE + SQUARE_SEP * (m - 1),
                        SEPARATOR_SIZE + SQUARE_SEP * (n - 1),
    ...
    private void displaySpots(Graphics2D g, int r, int c) {
        int num = _board.get(r, c).getSpots();

        if (num == 1) {
            spot(g, MID + SEPARATOR_SIZE + SQUARE_SEP * (c - 1),
                    MID + SEPARATOR_SIZE + SQUARE_SEP * (r - 1));
        } else if (num == 2) {
            spot(g, SPOT_MARGIN + SEPARATOR_SIZE + SQUARE_SEP * (c - 1),
                    OUT + SEPARATOR_SIZE + SQUARE_SEP * (r - 1));
            spot(g, OUT + SEPARATOR_SIZE + SQUARE_SEP * (c - 1),
                    SPOT_MARGIN + SEPARATOR_SIZE + SQUARE_SEP * (r - 1));
        } else if (num == 3) {
            spot(g, SPOT_MARGIN + SEPARATOR_SIZE + SQUARE_SEP * (c - 1),
                    OUT + SEPARATOR_SIZE + SQUARE_SEP * (r - 1));
            spot(g, OUT + SEPARATOR_SIZE + SQUARE_SEP * (c - 1),
            ...
            spot(g, OUT + SEPARATOR_SIZE + SQUARE_SEP * (c - 1),
                    SPOT_MARGIN + SEPARATOR_SIZE + SQUARE_SEP * (r - 1));
            spot(g, SPOT_MARGIN * 2 + SEPARATOR_SIZE + SQUARE_SEP * (c - 1),
                    EH + SEPARATOR_SIZE + SQUARE_SEP * (r - 1));
            spot(g, EH + SEPARATOR_SIZE + SQUARE_SEP * (c - 1),
                    SPOT_MARGIN * 2 + SEPARATOR_SIZE + SQUARE_SEP * (r - 1));
